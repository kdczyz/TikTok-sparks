...
from douyin_crawl4ai_v4 import DouyinCrawlerV3
...
async def fetch_qrcode(force: bool = False):
    ...
    try:
            ...
            try:
                ...
                if panel_alive:
                    ...
                    if hit:
                        ...
                        # 事件式等待：src 一变立即返回（v3 的 0.6s 间隔轮询
                        # 平均多等 ~3s，最坏白等 6s 才发现新码）
                        try:
                            await crawler.page.wait_for_function(
                                """(old) => {
                                    const img = document.querySelector(
                                        '#douyin_login_comp_scan_code img');
                                    return !!(img && img.src && img.src !== old);
                                }""",
                                arg=old_src, timeout=6000,
                            )
        ...
        await asyncio.sleep(0.5)  # 点完给渲染一点时间，实际就绪由 wait_for_panel 轮询保证
        ...
        if not panel.get("hasQR"):
            ...
            # 等 QR img 真正出现即可，不再固定 sleep(2)
            try:
                await crawler.page.wait_for_selector(
                    "#douyin_login_comp_scan_code img", timeout=5000)
            except Exception:
                pass

        # 等 QR img 完成渲染（面板出现后 img 常延迟加载，最多等 10s）
        # v4：先立即试一次，失败则事件式等 img 出现再试一次
        #（替代 v3 的固定 0.8s×12 轮询，平均快 ~3s，可靠性不变）
        success, img_bytes, err = await crawler.extract_qrcode()
        if not success:
            try:
                await crawler.page.wait_for_selector(
                    "#douyin_login_comp_scan_code img", timeout=10000)
            except Exception:
                pass
            success, img_bytes, err = await crawler.extract_qrcode()
...
def _detect_qr_in_png(data: bytes, pad: int = 24):
    ...
    性能要点：
    - 检测前把宽/高超过 ~1600px 的大图降采样（全页截图动辄 2880x1800，
      不降采样单次 detect 就要数秒），检测坐标按比例还原
    - 只对小图（<400px，如固定坐标裁剪）做 2x 放大重试，绝不放大全页图
      （旧逻辑对全页 2x 放大后检测 2000 万像素，是提取慢的主因）
    - 复用灰度图，避免重复转换

    检测重试链：原图 → （仅小图）2x 放大 → CLAHE 对比度增强
    """
    try:
        import numpy as _np
        import cv2
        ...
        # 大图先降采样再检测，坐标按 scale 还原
        MAX_DIM = 1600
        scale = 1.0
        if max(im.shape[:2]) > MAX_DIM:
            scale = MAX_DIM / max(im.shape[:2])
        ...
        gray = None
        # 重试链：原图 → 2x 放大（仅小图） → CLAHE 增强
        points = _find(im)
        if points is None and max(im.shape[:2]) < 400:
...
_last_face_capture_ts = 0.0   # 上次完整捕获尝试时间（节流用）


async def capture_verify_qrcode(crawler, panel_visible: bool = False,
                                force: bool = False) -> str:
    """
    截取刷脸验证页上"手机扫码进行刷脸"的二维码，返回 base64；失败返回 ""。

    通道（全部经过二维码内容校验，截不到真码就不返回）：
      1. 固定坐标裁剪（150x150，毫秒级）—— 仅在登录面板不可见时启用，
         防止把面板上旧登录二维码所在区域误当刷脸码
      2. 验证 iframe 整体截图（内含二维码）
      3. 全页截图 + OpenCV 定位二维码 → 精确裁剪（布局无关，最可靠但最慢）
    全部失败时把整页截图落盘 output/face_page_debug.png，便于诊断。

    性能（v4）：
    - 通道重排：最快的坐标裁剪先行，绝大多数情况毫秒级返回
    - 节流：距上次完整尝试 <1.5s 直接返回 ""（调用方沿用旧值），
      避免监控循环高频全页截图拖慢整个状态机；force=True 跳过节流
    """
    global _last_face_debug_ts, _last_face_capture_ts
    page = crawler._page

    # 节流：监控循环每次迭代都会调用，完整尝试限频 1.5s
    now = time.time()
    if not force and now - _last_face_capture_ts < 1.5:
        return ""
    _last_face_capture_ts = now

    # 1) 固定坐标裁剪（最快）：面板可见时跳过，防止误截旧登录二维码
    ...
    # 3) 全页截图 + QR 定位（最慢，兜底）：直接确认"截到的真的是二维码"再裁剪
    full_img = None
    try:
    ...
    while True:
        try:
            ...
            if _app["state"] == STATE_FACE_WAIT or (
                ...
                # panel_visible=True 时坐标裁剪通道会自动跳过（防误截旧登录码）
                fq = await capture_verify_qrcode(
                    crawler, panel_visible=bool(p.get("panelVis")))
    ...
    try:
        ...
        if method == "sms":
            ...
            try:
                ...
                # force=True：点击后必须立即捕获（不受监控循环节流影响）
                fq = await capture_verify_qrcode(crawler, force=True)
                if fq:
                    _app["face_qrcode_b64"] = fq
