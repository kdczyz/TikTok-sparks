...
class DouyinCrawlerV3:
    ...
    async def start_browser(self):
        ...
        # ── 持久化浏览器档案 ──
        # 每次全新 context 的指纹都会触发抖音"验证码中间页"（滑块拼图），
        # 使用 user_data_dir 持久化后重启 = 同一台设备，登录一次长期有效。
        profile_dir = Path(self.output_dir).parent / "browser_profile"
    ...
    async def open_douyin(self, url: str = None) -> bool:
        ...
        try:
            ...
            await asyncio.sleep(2.5)  # 等待 JS 渲染（不宜过长，调用方会轮询就绪）
            return True
